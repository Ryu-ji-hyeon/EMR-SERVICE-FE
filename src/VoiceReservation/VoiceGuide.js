...
const VoiceGuide = () => {
  ...
  const startVoiceGuide = () => {
    ...
    speak({
      text: '안녕하세요. 음성 안내 예약 시스템을 사용하기 전에 몇 가지 주의사항을 알려드리겠습니다. 이 시스템은 음성 인식 기술을 사용하여 예약을 진행하니, 정확한 인식을 위해 조용한 환경에서 사용해 주세요. 또한, 이 대화는 품질 개선을 위해 녹음될 수 있습니다. 예약을 진행하시겠습니까?',
      lang: 'ko-KR',
    });
  };
  ...
  const handleUserResponse = (response) => {
    ...
    // 여기서 인식된 텍스트(response)를 기반으로 다음 이벤트 처리
    if (response.includes('네')) {
      navigate('/Voice/DepartmentDoctorSelection'); // 예약 경로로 이동
    } else if (response.includes('아니오')) {
      navigate('/member/dashboard'); // 취소시 대시보드로 이동
    } else {
      setError('유효한 응답이 아닙니다. 다시 말해주세요.');
      speak({
        text: '유효한 응답이 아닙니다. 다시 말해주세요.',
        lang: 'ko-KR',
      });
    }
  };
